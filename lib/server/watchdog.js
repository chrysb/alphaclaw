...
const createWatchdog = ({
  ...
  const state = {
    ...
    configErrorRepairAttempted: false,
    safeMode: false,
    suppressedChannels: [],
    safeModeNotifiedKey: "",
  };
  ...
  const runHealthCheck = async ({
    ...
    if (parsed.ok) {
      ...
      state.awaitingAutoRepairRecovery = false;
      state.configErrorRepairAttempted = false;
    ...
    if (state.lifecycle === "config_error") {
      // The gateway is intentionally stopped on a fatal config error; a
      // failing probe adds no signal and must not demote the state or
      // trigger repairs beyond the one-shot config-error attempt.
      logEvent(
        "health_check",
        source,
        "ok",
        { reason: parsed.reason, skipped: true, configErrorActive: true },
        correlationId,
      );
      return false;
    }
  ...
  const onGatewayExit = ({
    ...
    if (code === kOpenclawConfigErrorExitCode) {
      // EX_CONFIG: the gateway refuses to run until the config is fixed.
      // Restarting in a loop would fight that contract, so attempt at most
      // one auto-repair per incident and otherwise stay stopped.
      const willAttemptRepair =
        state.autoRepair && !state.configErrorRepairAttempted;
      state.lifecycle = "config_error";
      state.health = "unhealthy";
      state.uptimeStartedAt = null;
      state.crashRecoveryActive = false;
      openIncident("gateway_config_error");
      logEvent(
        "config_error",
        "exit_event",
        "failed",
      ...
      void notifyOncePerIncident(
        willAttemptRepair ? "config_error_detected" : "config_error_paused",
        [
          "🐺 *AlphaClaw Watchdog*",
          withViewLogsSuffix(
            "🔴 Gateway stopped: fatal configuration error (exit 78)",
          ),
          "OpenClaw refuses to start until its configuration is fixed.",
          willAttemptRepair
            ? "Attempting one-time auto-repair (doctor --fix)..."
            : "Auto-restart paused; manual action required.",
        ].join("\n"),
        correlationId,
        "crash",
      );
      if (willAttemptRepair) {
        state.configErrorRepairAttempted = true;
        void runRepair({ source: "config_error", correlationId });
      }
      return;
    }
