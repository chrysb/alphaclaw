...
const registerPairingRoutes = ({
  ...
  app.get("/api/pairings", async (req, res) => {
    ...
    for (const ch of channels) {
      const sqliteRequests = readPendingChannelPairings({
        fsImpl: fsModule,
        openclawDir,
        channel: ch,
      });
      const pendingFromStore = (
        sqliteRequests === null
          ? readPendingPairingsFromStore({
              fsModule,
              openclawDir,
              channel: ch,
            })
          : sqliteRequests
              .map((entry) => mapPairingStoreEntry({ entry, channel: ch }))
              .filter(Boolean)
      );
  ...
  app.post("/api/pairings/:id/reject", (req, res) => {
    const channel = String(req.body?.channel || "telegram").trim().toLowerCase();
    const accountId = String(req.body?.accountId || "").trim();
    const pairingId = String(req.params.id || "").trim();
    if (!kAllowedPairingChannels.has(channel)) {
      return res.status(400).json({
    ...
    if (!pairingId || !kSafePairingArgPattern.test(pairingId)) {
      return res.status(400).json({ ok: false, error: "Invalid pairing id" });
    }
    if (accountId && !kSafePairingArgPattern.test(accountId)) {
      return res.status(400).json({ ok: false, error: "Invalid account id" });
    }
    try {
      const sqliteRemoved = removePendingChannelPairing({
        fsImpl: fsModule,
        openclawDir,
        channel,
        code: pairingId,
      ...
      const removed = sqliteRemoved === null
        ? removeRequestFromPairingStore({
            fsModule,
            openclawDir,
            channel,
            code: pairingId,
      ...
      pairingCache.ts = 0;
      if (removed) {
        console.log(`[alphaclaw] Rejected pairing request ${pairingId} for ${channel}${accountId ? `/${accountId}` : ""}`);
