...
const registerSystemRoutes = ({
  ...
  const isVisibleEnvKey = (key) => {
    const normalizedKey = String(key || "").trim();
    if (!normalizedKey) return false;
    if (isReservedUserEnvVar(normalizedKey) || isManagedChannelTokenKey(normalizedKey)) {
      return false;
    }
    if (!kKnownKeys.has(normalizedKey)) return true;
    const def = kKnownVars.find((entry) => entry.key === normalizedKey);
    return !!def && isVisibleInEnvars(def);
  };
  const redactEnvEntryValue = (key, value) => redactValue(key, value);
  const resolveEnvVarPlaceholders = (vars, existingVars) => {
    const existingByKey = new Map(
      (existingVars || []).map((entry) => [
        String(entry?.key || "").trim(),
        String(entry?.value || ""),
      ]),
    );
    return (vars || []).map((entry) => {
      const key = String(entry?.key || "").trim();
      const value = isSecretUnchangedValue(entry?.value)
        ? String(existingByKey.get(key) || "")
        : String(entry?.value || "");
  ...
  app.get("/api/env", (req, res) => {
    ...
    for (const def of kKnownVars) {
      ...
      const rawValue = fileEntry?.value || "";
      const redaction = redactEnvEntryValue(def.key, rawValue);
      merged.push({
        key: def.key,
        value: redaction.value,
        hasValue: redaction.hasValue,
        redacted: redaction.redacted,
        label: def.label,
        group: def.group,
        hint: def.hint,
        features: def.features,
        source: redaction.hasValue ? "env_file" : "unset",
        editable: true,
      });
    }
    ...
    for (const v of fileVars) {
      ...
      const redaction = redactEnvEntryValue(v.key, v.value);
      merged.push({
        key: v.key,
        value: redaction.value,
        hasValue: redaction.hasValue,
  ...
  app.get("/api/env/reveal", (req, res) => {
    const key = String(req.query?.key || "").trim();
    if (!key) {
      return res.status(400).json({ ok: false, error: "Missing key" });
    }
    if (!isVisibleEnvKey(key)) {
      return res.status(404).json({ ok: false, error: "Environment variable not found" });
    }
    const entry = readEnvFile().find((item) => item.key === key);
  ...
  app.put("/api/env", (req, res) => {
    ...
    const resolvedVars = resolveEnvVarPlaceholders(vars, existingEnvVars);
    const filtered = resolvedVars.filter(
      (v) => !isReservedUserEnvVar(v.key) && !isManagedChannelTokenKey(v.key),
    );
    const existingLockedVars = existingEnvVars.filter((v) =>
      isReservedUserEnvVar(v.key),
    );
    const existingManagedChannelVars = existingEnvVars.filter((v) =>
      isManagedChannelTokenKey(v.key),
    );
    ...
    const existingHiddenKnownVars = existingEnvVars.filter((v) =>
