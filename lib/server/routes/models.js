const { kClawRuntime, kFallbackOnboardingModels } = require("../constants");
...
const registerModelRoutes = ({
  ...
  app.get("/api/models/status", async (req, res) => {
    try {
      const output = await shellCmd(
        kClawRuntime.buildCommand("models status --json"),
        {
          env: gatewayEnv(),
          timeout: 20000,
        },
  ...
  app.post("/api/models/set", async (req, res) => {
    ...
    try {
      await shellCmd(kClawRuntime.buildCommand(`models set "${modelKey}"`), {
        env: gatewayEnv(),
        timeout: 30000,
      });
