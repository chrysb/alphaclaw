...
const getGatewayLaunchAgentPath = () =>
  path.join(os.homedir(), "Library", "LaunchAgents", "ai.openclaw.gateway.plist");

const shouldUseGatewayServiceManager = () =>
  process.platform === "darwin" && fs.existsSync(getGatewayLaunchAgentPath());

const getGatewayLaunchAgentLabel = () => "ai.openclaw.gateway";

const isGatewayServiceLoaded = () => {
  if (!shouldUseGatewayServiceManager()) return false;
  try {
    execSync(`launchctl print gui/${process.getuid()}/${getGatewayLaunchAgentLabel()}`, {
      stdio: "ignore",
      timeout: 5000,
    });
    return true;
...
const runGatewayCmd = async (cmd) => {
  ...
  if (cmd === "--force") {
    const ok = await runGatewayRestartCmd("--force");
    return { ok, mode: "managed_supervisor" };
  }
  return runGatewayShortCmd(cmd);
};
...
const ensureGatewayRunning = async ({
  allowRestartLoadedService = false,
} = {}) => {
  if (!isOnboarded()) {
    console.log("[alphaclaw] Not onboarded yet — skipping gateway start");
    return { ok: false, skipped: true, reason: "not_onboarded" };
  }
  ...
  if (shouldUseGatewayServiceManager()) {
    if (isGatewayServiceLoaded()) {
      if (!allowRestartLoadedService) {
        console.log(
          "[alphaclaw] Gateway LaunchAgent already loaded — skipping service start",
        );
        return { ok: true, skipped: true, reason: "service_loaded" };
      }
      const result = await runGatewayCmd("restart");
      return {
        ...result,
        mode: "service_restart",
      };
    }
    const result = await runGatewayCmd("start");
    return {
      ...result,
      mode: "service_start",
    };
...
const startGateway = async () =>
  ensureGatewayRunning({ allowRestartLoadedService: false });

const recoverGateway = async () =>
  ensureGatewayRunning({ allowRestartLoadedService: true });

const restartGateway = async (reloadEnv) => {
  reloadEnv();
  stopManagedGatewayChild();
  await runGatewayCmd("--force");
};

const restartGatewayLight = async (reloadEnv) => {
  reloadEnv();
  markManagedGatewayExitExpected();
  await runGatewayCmd("restart");
};

const handleGatewayShutdownSignal = (signal) => {
  // The gateway is a shared runtime service and may be launchd-owned.
  // Do not unload it just because the AlphaClaw UI is exiting.
  console.log(
    `[alphaclaw] Received ${signal}; leaving gateway service running and exiting UI process`,
  );
  process.exit(0);
};
...
const attachGatewaySignalHandlers = () => {
  process.on("SIGTERM", () => {
    handleGatewayShutdownSignal("SIGTERM");
  });
  process.on("SIGINT", () => {
    handleGatewayShutdownSignal("SIGINT");
  });
};
...
module.exports = {
  ...
  getGatewayPort,
  getGatewayUrl,
  getGatewayLaunchAgentPath,
  getGatewayLaunchAgentLabel,
  isOnboarded,
  isGatewayRunning,
  isGatewayServiceLoaded,
