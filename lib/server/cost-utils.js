...
const selectPricingRates = (pricing, promptTokens) => {
  if (!Array.isArray(pricing?.tieredPricing)) return pricing;
  const tier = pricing.tieredPricing.find(({ range }) => {
    if (!Array.isArray(range) || !Number.isFinite(range[0])) return false;
    const end = Number.isFinite(range[1]) ? range[1] : Infinity;
    return promptTokens >= range[0] && promptTokens < end;
  });
  return tier || pricing;
};
...
const deriveCostBreakdown = ({
  ...
  const fallbackPricing = resolvePricingFromFallbackMap(model);
  const discoveredPricing = resolvePricingFromOpenclawNodeModules({
    provider,
    model,
  });
  const pricing = fallbackPricing?.tieredPricing
    ? fallbackPricing
    : discoveredPricing || fallbackPricing;
  if (!pricing) {
    return {
      inputCost: 0,
      outputCost: 0,
  ...
  const rates = selectPricingRates(pricing, promptTokens);
  const inputRate = resolvePerMillionRate(rates.input, inputTokens);
  const outputRate = resolvePerMillionRate(rates.output, outputTokens);
  ...
  const cacheWriteRate = resolvePerMillionRate(
    rates.cacheWrite == null ? rates.input : rates.cacheWrite,
    cacheWriteTokens,
  );
