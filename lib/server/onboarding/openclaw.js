...
const kDefaultSessionVisibility = "agent";
const kDefaultSubagentSpawnDepth = 1;
const kBootstrapAgentsPath = "hooks/bootstrap/AGENTS.md";
const kLegacyBootstrapToolsPath = "hooks/bootstrap/TOOLS.md";

const reconcileBootstrapExtraFilesEntry = (entry = {}) => {
  const configuredPaths = [entry.paths, entry.patterns, entry.files]
    .flatMap((value) => (Array.isArray(value) ? value : []))
    .map((value) => String(value || "").trim())
    .filter(Boolean);
  const userPaths = configuredPaths.filter(
    (value) =>
      value !== kBootstrapAgentsPath &&
      value !== kLegacyBootstrapToolsPath,
  );
  const reconciled = {
    ...entry,
    enabled: true,
    paths: [kBootstrapAgentsPath, ...new Set(userPaths)],
  };
  delete reconciled.patterns;
  delete reconciled.files;
  return reconciled;
};
...
const ensureManagedConfigShell = (cfg, { openAiCompatApiEnabled = false } = {}) => {
  ...
  if (!cfg.gateway.cliAgents) cfg.gateway.cliAgents = {};
  ...
  if (cfg.tools.sessions.visibility === undefined) {
    cfg.tools.sessions.visibility = kDefaultSessionVisibility;
  }
  if (cfg.tools.swarm === undefined) cfg.tools.swarm = false;
  ...
  if (cfg.agents.defaults.subagents.maxSpawnDepth === undefined) {
    cfg.agents.defaults.subagents.maxSpawnDepth = kDefaultSubagentSpawnDepth;
  }
