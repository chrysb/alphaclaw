...
const createOnboardingService = ({
  ...
  const clawRuntime = constants.kClawRuntime || {
    buildCommand: (cmd) => `openclaw ${String(cmd || "").trim()}`.trim(),
  ...
  const completeOnboarding = async ({
    ...
    if (!existingConfigPresent) {
      const onboardArgs = buildOnboardArgs({
        ...
        hasCodexOauth,
        workspaceDir: WORKSPACE_DIR,
        gatewayPort: clawRuntime.defaultGatewayPort,
      });
      await shellCmd(
        clawRuntime.buildCommand(
          `onboard ${onboardArgs.map((a) => `"${a}"`).join(" ")}`,
        ),
        {
          env: gatewayEnv(),
          timeout: 120000,
    ...
    await shellCmd(clawRuntime.buildCommand(`models set "${modelKey}"`), {
      env: gatewayEnv(),
      timeout: 30000,
    }).catch((e) => {
    ...
    startGateway();
    startDenchWebRuntime?.({ reason: "onboarding" })?.catch?.((error) => {
      console.error(
        `[onboard] Failed to start DenchClaw web runtime: ${error.message}`,
      );
    });
