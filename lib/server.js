...
const { createDenchWebRuntime } = require("./server/dench-web-runtime");
const { attachProxyBodyReplay } = require("./server/proxy-body-replay");
...
const denchWebRuntime = createDenchWebRuntime({
  clawRuntime: constants.kClawRuntime,
  gatewayEnv,
});
...
const { isAuthorizedRequest, gmailWatchService } = registerServerRoutes({
  ...
  getBaseUrl,
  startGateway,
  startDenchWebRuntime: (options) => denchWebRuntime.start(options),
  syncChannelConfig,
  getChannelStatus,
