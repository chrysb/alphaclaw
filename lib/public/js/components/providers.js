...
const getEnvVarEntry = (vars, key) => vars.find((v) => v.key === key) || null;
...
export const Providers = ({ onRestartRequired = () => {} }) => {
  ...
  const renderCredentialField = (field) => {
    const envEntry = getEnvVarEntry(envVars, field.key);
    const fieldValue = envEntry?.value || "";
    return html`
      <div class="space-y-1">
        <div class="flex items-center gap-3">
          <label class="text-xs font-medium text-fg-muted">${field.label}</label>
          ${field.url && !fieldValue
            ? html`<a
                href=${field.url}
                target="_blank"
        ...
        <${SecretInput}
          ...
          onReveal=${envEntry?.redacted
            ? async () => {
                const result = await revealEnvVar(field.key);
                return result.value || "";
              }
            : undefined}
          onRevealError=${(err) =>
            showToast(
              `Could not reveal ${field.label}: ${err.message || err}`,
              "error",
            )}
          placeholder=${field.placeholder || ""}
          isSecret=${!field.isText && (!!fieldValue || !!envEntry?.hasValue)}
