...
import htm from "htm";
import { fetchEnvVars, revealEnvVar, saveEnvVars } from "../lib/api.js";
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-1">
          <${SecretInput}
            ...
            onRevealError=${(err) =>
              showToast(
                `Could not reveal ${envVar.key}: ${err.message || err}`,
                "error",
              )}
            placeholder=${envVar.value ? "" : "not set"}
            isSecret=${!!envVar.value || !!envVar.hasValue}
