  ...
  return html`
    <div class="bg-surface border border-border rounded-xl p-4 space-y-3">
      ...
      ${credentialModes.map((mode) => {
        ...
        const fieldRedacted = !!currentValue?.[`${mode.field}Redacted`];
        const isActive =
          !hasMultipleModes ||
        ...
        return html`
          <div class="space-y-1.5">
            ...
            <${SecretInput}
              ...
              onReveal=${fieldRedacted
                ? () => onRevealProfileSecret(profileId, mode.field)
                : undefined}
              onRevealError=${(err) =>
                showToast(
                  `Could not reveal ${mode.label}: ${err.message || err}`,
                  "error",
                )}
