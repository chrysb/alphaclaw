...
export const revealModelAuthSecret = async ({
  profileId,
  field,
  agentId,
} = {}) => {
  const params = new URLSearchParams({ field: String(field || "") });
  if (agentId) params.set("agentId", agentId);
  const res = await authFetch(
    `/api/models/auth/${encodeURIComponent(profileId)}/reveal?${params.toString()}`,
  );
  return parseJsonOrThrow(res, "Could not reveal auth secret");
};
...
export async function revealEnvVar(key) {
  ...
  const res = await authFetch(`/api/env/reveal?${params.toString()}`);
  return parseJsonOrThrow(res, "Could not reveal environment variable");
}
